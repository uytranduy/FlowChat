...
export const updateGroupInvitePermission = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (conversation.group?.createdBy?.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: "Chỉ trưởng nhóm mới có thể thay đổi quyền mời" });
    }
    if (typeof req.body?.allowMembersToInvite !== "boolean") {
      return res.status(400).json({ message: "Quyền mời thành viên không hợp lệ" });
    }
    await Conversation.updateOne(
      { _id: conversation._id },
      { $set: { "group.allowMembersToInvite": req.body.allowMembersToInvite } }
    );
    const formatted = await presentConversation(conversation._id.toString());
    if (!formatted) return res.status(404).json({ message: "Không tìm thấy cuộc trò chuyện" });
    io.to(conversation._id.toString()).emit("conversation:updated", formatted);
    return res.status(200).json({ message: "Đã cập nhật quyền mời thành viên", conversation: formatted });
  } catch (error) {
    console.error("Lỗi khi cập nhật quyền mời thành viên", error);
    return res.status(500).json({ message: "Lỗi hệ thống" });
  }
};
